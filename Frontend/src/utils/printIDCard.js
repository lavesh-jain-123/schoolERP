// Helper function to generate a single card's HTML (front + back)
    ...
    <style>
      ...
      * {
      ...
      .card-side {
        ...
        background: linear-gradient(135deg, #2c5f93 0%, #2a3f73 100%);
        position: relative;
...
// Original single ID card print function
...
// New bulk ID cards print function
export const printBulkIDCards = (students) => {
  if (!students || students.length === 0) {
    throw new Error('No students provided for bulk printing');
  }

  const printWindow = window.open('', '_blank', 'width=1000,height=800');
  
  // Generate HTML for all students
  const allCardsHTML = students.map(student => generateSingleCardHTML(student)).join('\n');
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Bulk ID Cards - ${students.length} Students</title>
      ${getSharedStyles()}
    </head>
    <body>
      <div class="bulk-info no-print">
        📋 ${students.length} ID Card${students.length !== 1 ? 's' : ''}
      </div>
      <button class="print-button no-print" onclick="window.print()">🖨️ Print All Cards</button>
      
      ${allCardsHTML}
      
      <script>
        // Log student count for debugging
        console.log('Bulk ID Cards loaded: ${students.length} students');
